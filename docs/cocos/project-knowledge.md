...
- **隐藏 UI 的组件生命周期时机（已踩坑验证）**：场景里默认 `active=false` 的节点，其组件 `start()`/`onLoad()` 会推迟到**首次激活**时才执行。弹出式面板因此不能在 `start()` 里再把自己 `active=false`——`show()` 首次激活的同一瞬间会被自己藏回去（表现为"结算面板永远不出现"，2026-02 修复 `ResultPanel`）。隐藏态节点上的按钮接线放在 `start()` 里是安全的：首次激活后一帧内完成，玩家点不到该间隙。
